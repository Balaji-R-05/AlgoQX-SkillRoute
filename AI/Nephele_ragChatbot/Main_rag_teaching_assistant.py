...
class Config:
    ...
    huggingface_api_key: str = os.getenv("HUGGINGFACE_API_KEY", os.getenv("HUGGINGFACEHUB_API_TOKEN", os.getenv("HF_TOKEN", "")))
...
LESSON_FILE = os.path.join(BASE_DIR, "project_folder", "last_lesson.txt")
AUDIO_FILES_DIR = os.path.join(BASE_DIR, "project_folder", "audio_files")
    ...
    def setup_provider(self):
        if self.config.llm_provider == "openai":
            ...
            self.client = openai.OpenAI(
                base_url="http://100.87.204.58:11434/v1",
...
class SpeechRecognizer:
    def __init__(self, config: Config):
        ...
        if config.groq_api_key:
            ...
            print("⚠️ GROQ_API_KEY not found. Audio transcription via Whisper disabled (Text Mode Active).")
        ...
        try:
            ...
            print(f"⚠️ PyAudio/Microphone not found: {e}. Voice input is disabled (Text only mode active).")
    ...
    def listen_for_speech(self, prompt: str = "Listening...", timeout: int = None, phrase_timeout: int = None) -> Optional[str]:
        if not getattr(self, "microphone", None):
            return input(f"{prompt} [Type your response]: ").strip()
...
class TeachingAssistant:
    ...
    async def speak_and_play(self, text: str):
        audio = await self.audio_manager.speak_text(text)
        if audio and os.path.exists(audio):
            self.audio_manager.play_audio_blocking(audio)
    ...
    def is_question_related_with_llm(self,question, lesson_filename, api_key, model="gpt-3.5-turbo"):
        ...
        answer = self.llm_provider._call_llm(prompt).strip().lower()
    ...
    async def final_doubt_clearing_session(self):
        print("\n" + "=" * 60)
        print("🤔 FINAL DOUBT CLEARING SESSION")
        print("=" * 60)
        doubt_announcement = "If you have any doubts or questions about today's lesson, please share them now."
        print(f"🎙️ {doubt_announcement}")
        await self.speak_and_play(doubt_announcement)
        ...
        for attempt in range(2):
            ...
            if mode == 'q':
                ...
                await self.speak_and_play("Doubt session ended. Let's move to the next topic.")
                return
            ...
            if doubt_question:
                ...
                if is_relevant:
                    ...
                    await self.speak_and_play(explanation)

                else:
                    not_relevant_msg = "This topic will be covered in future lessons."
                    print(not_relevant_msg)
                    await self.speak_and_play(not_relevant_msg)
            ...
            if attempt == 0:
                ...
                second_chance_text = "Last chance to share any doubts or questions."
                print(f"🎙️ {second_chance_text}")
                await self.speak_and_play(second_chance_text)
            else:
                print("⏱️ No doubts received in second window either.")

        final_message = "Great! It seems all concepts are clear. Let's move on to the next topic."
        print(f"🎙️ {final_message}")
        await self.speak_and_play(final_message)
        print("\n✅ Doubt clearing session completed!")
    ...
    async def run(self):
        ...
        try:
            ...
            while True:
                ...
                lesson_success = await self.teach_lesson(topic)
                if lesson_success:
                    # self.interactive_qa()
                    await self.final_doubt_clearing_session()
                print("\n" + "="*50)
